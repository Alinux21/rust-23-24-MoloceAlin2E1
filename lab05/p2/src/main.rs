...
fn new_canvas()->Canvas{
    let canvas:Canvas=Canvas{surface: [[' ';50];10]};
    canvas

}

fn set_pixels(c:&mut Canvas,m:&[(u32,u32,u8)]){
    let i=m[0].0 as usize;
    let j=m[0].1 as usize;
    c.surface[i][j]= m[0].2 as char;
}

fn print(c:Canvas){
    for i in 0..c.surface.len(){
        for j in 0..c.surface[i].len(){
            print!("{} ",c.surface[i][j]);
        }
        println!();
    }
